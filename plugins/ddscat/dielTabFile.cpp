	...
	namespace io {
		namespace ddscat {
				...
				if (fname == "")
				{
					if (_filename.size())
					{
						...
						ICEDB_throw(error::error_types::xBadInput)
							.add<std::string>("Reason", "Must specify filename");
					}
				}
			...
			void dielTab::readString(const std::string &in)
			{
					...
					for (size_t i = 0; i < 3; i++)
					{
							...
							// Read in five unsigned ints, then assign.
						{
							...
							for (size_t j = 0; j < 5; j++)
							{
								...
								v[j] = boost::lexical_cast<size_t>(s);
				...
				if (!importMap[0]) ICEDB_throw(error::error_types::xBadInput)
					.add<std::string>("Reason", "Cannot make diel map without "
						"wavelengths.")
					.add<std::string>("Error-line-text", in);
				// eps = m^2
				...
				if (!sufficient) ICEDB_throw(error::error_types::xBadInput)
					.add<std::string>("Reason", "Cannot make diel map with incomplete information.")
					.add<std::string>("Error-line-text", in);
			...
			void dielTab::write(std::ostream &out) const
			{
				...
				if (!freqMMap.size()) ICEDB_throw(error::error_types::xArrayOutOfBounds)
					.add<std::string>("Reason", "freqMMap is empty");
				if (!_colMapsValid()) ICEDB_throw(error::error_types::xBadInput)
					.add<std::string>("Reason", "Bad diel column mappings");
			...
			std::complex<double> dielTab::interpolate(double freq) const
			{
				using namespace std;
				complex<double> res;
				if (freqMMap.size() == 0) ICEDB_throw(error::error_types::xArrayOutOfBounds)
					.add<std::string>("Reason", "freqMMap is empty")
					.add<double>("Frequency", freq);
