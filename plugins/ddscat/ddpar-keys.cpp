	...
	namespace io {
		namespace ddscat {
			...
			void ddPar::getSIJ(std::set<size_t> &sij) const
			{
				...
				std::shared_ptr< const ddParParsers::ddParLineSimplePlural<size_t> > line;
				std::shared_ptr< const ddParParsers::ddParLine > linein;
				std::vector<size_t> val;
				// NSMELTS, INDICESIJ
				getKey(ddParParsers::INDICESIJ, linein);
				line = std::static_pointer_cast<const ddParParsers::ddParLineSimplePlural<size_t>>(linein);
				line->get(val);
			...
			void ddPar::setSIJ(const std::set<size_t> &sij)
			{
				...
				std::shared_ptr<ddParParsers::ddParLineSimple<std::size_t> > line_size
				(new ddParParsers::ddParLineSimple<std::size_t>(ddParParsers::NSMELTS));
				line_size->set(sij.size());

				// Set array
				std::shared_ptr< ddParParsers::ddParLineSimplePlural<size_t> > line
				...
				insertKey(ddParParsers::NSMELTS, std::static_pointer_cast<ddParParsers::ddParLine>(line_size));
				insertKey(ddParParsers::INDICESIJ, std::static_pointer_cast<ddParParsers::ddParLine>(line));
			}
			...
			void ddPar::getAeff(double &min, double &max, size_t &n, std::string &spacing) const
			{
				...
				line = std::static_pointer_cast<const ddParParsers::ddParLineMixed<double, std::string> > (linein);
				line->setSep(3, 4);
			...
			void ddPar::setAeff(double min, double max, size_t n, const std::string &spacing)
			{
				...
				insertKey(ddParParsers::AEFF, (line));
			}
			...
			void ddPar::setWavelengths(double min, double max, size_t n, const std::string &spacing)
			{
				...
				line->set<double>(2, (double)n);
				line->set<std::string>(3, spacing);
				insertKey(ddParParsers::WAVELENGTHS, (line));
			}
			...
			{
				std::shared_ptr<const ddParParsers::ddParLineSimplePlural<double> > res;
				getPlane(n, res);
				res->get(0, phi);
			...
			void ddPar::delPlane(size_t key)
			{
				...
				std::shared_ptr<ddParParsers::ddParLineSimple<std::size_t> > nplanes
			...
			{
				std::shared_ptr<ddParParsers::ddParLineSimplePlural<double> > res
			...
			{
				//std::shared_ptr<const ddParParsers::ddParLineSimple<std::string> > res;
			...
			void ddPar::setDiels(const std::vector<std::string>& src)
			{
				...
				for (auto &file : src)
				{
					std::shared_ptr<ddParParsers::ddParLineSimple<std::string> > res
				...
				std::shared_ptr< ddParParsers::ddParLineSimple<int> > line
				(new ddParParsers::ddParLineSimple<int>(ddParParsers::NCOMP));
				line->set((int)_diels.size());
				insertKey(ddParParsers::NCOMP, (line));
			}
			...
			void ddPar::populateDefaults(bool overwrite, const std::string &src) const
			{
				...
				using namespace ddscat::ddParParsers;
				...
				std::shared_ptr<ddPar> basep; // basep is a copy with auto-deletion
				if (src == "")
				{
					basep = ddPar::generate(defaultInstance());
					//basep = std::shared_ptr<ddPar>(new ddPar(*(defaultInstance())));
				}
				else
				{
					if (boost::filesystem::exists(path(src)))
					{
						basep = ddPar::generate(src); //std::shared_ptr<ddPar>(new ddPar(src));
					}
					else {
						ICEDB_throw(error::error_types::xMissingFile)
							.add<std::string>("Reason", "missing file")
							.add<std::string>("Filename", src);
					}
				}
				ddPar &base = *basep;
