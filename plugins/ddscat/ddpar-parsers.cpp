...
namespace icedb {
	namespace io {
		namespace ddscat {
			...
			{
				std::shared_ptr<const ddParParsers::ddParLineSimple<T> > line;
				std::shared_ptr< const ddParParsers::ddParLine > linein;
				T v;
				getKey(key, linein);
				line = std::static_pointer_cast<const ddParParsers::ddParLineSimple<T>>(linein);
				line->get(v);
				return v;
			}
			...
			{
				std::shared_ptr< ddParParsers::ddParLineSimple<T> >
					line(new ddParParsers::ddParLineSimple<T>(key));
				line->set(val);
				insertKey(key, std::static_pointer_cast<ddParParsers::ddParLine>(line));
			}
			...
			bool ddPar::__getSimpleBool(ddParParsers::ParId key) const
			{
				...
				size_t v;
				getKey(key, linein);
				line = std::static_pointer_cast<const ddParParsers::ddParLineSimple<size_t>>(linein);
				line->get(v);
				return (v) ? true : false;
			}
			...
			{
				std::shared_ptr< ddParParsers::ddParLineSimple<size_t> >
			...
			valtype ddPar::__getSimplePlural(ddParParsers::ParId id, size_t index) const
			{
				...
				valtype v;
				getKey(id, linein);
				line = std::static_pointer_cast<const ddParParsers::ddParLineSimplePlural<valtype>>(linein);
				line->get(index, v);
				return v;
			}
			...
			void ddPar::__setSimplePlural(ddParParsers::ParId id, size_t index, size_t maxSize, const valtype &v)
			{
				...
				std::shared_ptr< ddParParsers::ddParLine > linein;
				// does key exist? if so, load it and prepare for update
				if (exists(id))
				{
					getKey(id, linein);
					line = std::static_pointer_cast<ddParParsers::ddParLineSimplePlural<valtype>>(linein);
				}
				else {
					line = std::shared_ptr<ddParParsers::ddParLineSimplePlural<valtype> >
			...
			{
				std::shared_ptr< ddParParsers::ddParTuples<valtype> > line;
				std::shared_ptr< ddParParsers::ddParLine > linein;
				// does key exist? if so, load it and prepare for update
				if (exists(id))
				{
					getKey(id, linein);
					line = std::static_pointer_cast<ddParParsers::ddParTuples<valtype>>(linein);
				}
				else {
					line = std::shared_ptr<ddParParsers::ddParTuples<valtype> >
			...
			void ddPar::__getString(ddParParsers::ParId id, std::string &val) const
			{
				...
				std::string v;
				getKey(id, linein);
				line = std::static_pointer_cast<const ddParParsers::ddParLineSimple<std::string>>(linein);
				line->get(v);
				val = v;
			}
			...
			namespace ddParParsers
			{
				std::shared_ptr<ddParLine> mapKeys(const std::string &key)
				{
					...
					// IWRPOL is version 7.0
						...
						ptr = std::shared_ptr<ddParLineSimplePlural<int> >
						...
						ptr = std::shared_ptr<ddParLineSimplePlural<std::size_t> >
					...
					else
					{
						...
						ICEDB_throw(error::error_types::xBadInput)
							.add<std::string>("Reason", "Unmatched key")
				...
				{
					if (ddscat::ddVersions::isVerWithin(ver, 72, 0))
					{
						switch (_id)
						{
						case IWRPOL:
					...
					if (ddscat::ddVersions::isVerWithin(ver, 0, 72)) // up through 7.1 (inclusive)
					{
						switch (_id)
						{
						case NRFLD:
				...
				void idString(ParId id, std::string &key, size_t version)
				{
					...
					default:
						ICEDB_throw(error::error_types::xBadInput)
							.add<std::string>("Reason", "Unmatched parid");
					}
				}
			} // end ddparparsers
