...
namespace HH {
	...
	namespace Tags {
		namespace PropertyLists {
			...
			//template <class T> struct tag_fill_value {};
			template <class T> using t_FillValue = Tag<_detail::tag_fill_value<T>, T>;
		...
		public:
			...
			**/
			...
			PL setDatasetCreationPList(std::tuple<Args...> vals)
			{
				...
				t_DoShuffle doShuffle(false); getOptionalValue(doShuffle, vals);
				t_CompressionType compressionType(CompressionType::NONE); getOptionalValue(compressionType, vals);
				t_GZIPlevel gz_level(6); getOptionalValue(gz_level, vals);
				t_SZIP_PixelsPerBlock sz_ppb(16); getOptionalValue(sz_ppb, vals);
				t_SZIPopts sz_opts(H5_SZIP_EC_OPTION_MASK); getOptionalValue(sz_opts, vals);
				...
				if (hasCompression) {
					...
					else if (compressionType.data == CompressionType::SZIP) addSZIP();
					else if (compressionType.data == CompressionType::GZIP) addGZIP();
				...
				*/
				...
				if (hasManualChunking) {
					...
					Expects(0 <= H5Pset_chunk(base(), (int)manualChunking.data.size(), manualChunking.data.begin()));
				}

				// Fill value
				if (hasFillValue) {
					t_FillValue<DataType> fill; getOptionalValue(fill, vals);
			...
			PL setFileAccessPList(std::tuple<Args...> vals)
			{
				...
				if (hasSetCache) {
					t_FileCacheParams cps; getOptionalValue(cps, vals);
					Expects(0 <= H5Pset_cache(base(),
						0,
						cps.data.rdcc_nslots,
						cps.data.rdcc_nbytes,
						cps.data.rdcc_w0
					));
				}
