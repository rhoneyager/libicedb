	...
	private:
		...
		Attribute _read(HH_hid_t in_memory_dataType, size_t flsize, gsl::span<DataType> data, bool isvlen) const
		{
			...
			if (!isvlen) {
				...
				herr_t ret = H5Aread(attr(), in_memory_dataType(), (void*)p->DataPointers.data());
				if (ret < 0) throw HH_throw.add("Reason", "H5Aread failed.");
				m.deserialize(p, data);
				//herr_t ret = H5Aread(attr(), in_memory_dataType(), (void*)data.data());
				//if (ret < 0) throw HH_throw.add("Reason", "H5Aread failed.");
			}
			else {
				// When read, we get pointers to the variable-length objects.
				Marshaller m;
				auto p = m.prep_deserialize(flsize);
				herr_t ret = H5Aread(attr(), in_memory_dataType(), (void*)p->DataPointers.data());
				if (ret < 0) throw HH_throw.add("Reason", "H5Aread failed.");
