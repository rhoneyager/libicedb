...
namespace HH {
	...
	public:
		...
		template <class T> Error& add(const std::string& key, const T value) {
			if (!stk.size()) push();
	...
	inline void fail_fast_assert(bool cond, const ::std::string &message, const ::std::string& fn, int line, const ::std::string& sf)
	{
		if (!cond)
			throw ::HH::Error().push().add("Reason", std::string(message))
...
#define HH_Expects( x )  ::HH::fail_fast_assert( (x), ::std::string("HH: Assertion failure: ").append(::std::string(#x)), ::std::string(__FILE__), (int)__LINE__, ::std::string(HH_DEBUG_FSIG));
