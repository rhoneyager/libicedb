...
namespace icedb {
	...
	namespace zeros {
		...
		ICEDB_DL double findzero(double a, double b, const std::function<double(double) > & evaltarget);
			...
			double eps = 0.000001, size_t maxIter = 50)
		{
			...
			if (i >= maxIter) ICEDB_throw(icedb::error::error_types::xModelOutOfRange)
