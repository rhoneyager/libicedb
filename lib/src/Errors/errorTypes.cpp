#include "icedb/Errors/errorTypes.hpp"
namespace icedb {
	namespace error {
		const char* stringify(error_types et) {
			...
			else if (et == error_types::xDLLerror) return "Unspecified DLL ICEDB_LOG_ERROR.";
			...
			else if (et == error_types::xOtherError) return "Nonspecific ICEDB_LOG_ERROR.";
			else if (et == error_types::xUnsupportedIOaction) return "Unsupported IO action.";
			else return "INVALID - stringify does not have the appropriate ICEDB_LOG_ERROR type!";
		}
	}
}
