...
// First is the compiled icedb version info.
// Second is rdcheck - the plugin-reported address &(icedb::registry::dump_hook_table).
...
#define icedb_plugin_init(x) d_dllVer(x); //gcc_init();
#else
#define icedb_plugin_init(x) d_dllVer(x); //msvc_init();
#endif
...
namespace icedb
{
	namespace registry
	{
		...
		template <class T>
		std::shared_ptr<icedb::registry::IOhandler> read_file_type_iterate
			(std::shared_ptr<icedb::registry::IOhandler> sh, 
			std::shared_ptr<icedb::registry::IO_options> opts,
